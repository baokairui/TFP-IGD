...
def get_A(nx,ny,a,b,r,m):
    ...
    d01[-1] = -2
    d02 = - np.ones(nx) * 2
    d02 = np.tile(d02, ny-1)
    d0 = np.concatenate((d01,d02),axis=0)
    d0 =  d0 * (a + r) 
    ...
    d13 = np.ones(nx-1) 
    d13[0] = 0
    d1 = np.concatenate((-d11 * m[0],d12,np.zeros(nx-1)),axis=0)
    d1 = d1 * a[:-1]
    d13_ = np.ones(nx-1) 
    d13_[-1] = 0
    d1_ = np.concatenate((d13_ * m[0][1:],d12,np.zeros(nx)),axis=0)
    d1_ = d1_ * a[1:]
    ...
    d3 = d3 * r[:-nx]
    d3_ = np.concatenate((d32,np.zeros(nx)),axis=0)
    d3_ = d3_ * r[nx:]
    ...
    A = scipy.sparse.diags([d0, d1, d1_, d2, d2_, d3, d3_, d4, d4_, d5], [0, 1, -1, nx-1, -nx+1, nx, - nx, nx+1, -nx-1, 2*nx], format='csc')
    return A

def get_b(nx,ny,q,n,T_left = 40,T_right = 40,T_down = 40):
    b = np.zeros((ny,nx))
    b[0] = - n[0] * q
    b[-1] = - T_down * 4
    b[:,0] = - T_left * 4
...
def solve(nx,ny,data_path1,data_path2,data_path3,data_path4,level):
    ...
    for i in range(len(alpha)):
        ...
        # print(t)
        T[i] = t
    sio.savemat('./dataset/2000/T/Neuman/T'+str(level)+'.mat',{'b':T})
...
if __name__ == '__main__':
    for i in range(1,5):
        ...
        ny = 4 * ( 2 ** i )
        level = 5 - i
        solve(nx,ny,'./dataset/2000/alpha/alpha'+str(level),'./dataset/2000/beta/beta'+str(level),'./dataset/2000/gamma/gamma'+str(level),'./dataset/2000/J/J'+str(level),level)
