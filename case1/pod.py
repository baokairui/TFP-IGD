...
class ML_POD(object):
    ...
    def multilevel(self):
        total_mean = []
        total_base = []
        coeff, data_mean, base = ML_POD.pod(self,self.dataset_path+'/T'+str(3)+'.mat')
        total_mean.append(data_mean)
        total_base.append(base)
        ML_POD.predict(self,self.predata_path,coeff,3)
        for i in range (2,3-self.level,-1):
            ...
            ML_POD.predict(self,self.predata_path,coeff_cal,3-i)
...
class get_new_pre(object):
    ...
    def load_model(self):
        model = []
        temp = joblib.load(self.model_path+'/ExtraTrees_sim3.pkl' )
        model.append(temp)
    ...
    def multilevel_result(self,model_total,mean,base):
        nx = int(128 / 2 ** (3-self.level))
        ny = int(32 / 2 ** (3-self.level))
    ...
    def multilevel_resultshow(self,model_total,mean,base):
        plt.figure()
        nx = int(128 / 2 ** (4-self.level))
        ny = int(32 / 2 ** (4-self.level))
        result = get_new_pre.model_result(self,model_total[0],self.pre_data,mean[0],base[0])
        plt.subplot(2,2,1)
...
if __name__ == '__main__':
    ...
    get_pod_path = ML_POD(datasize,f'./dataset/{datasize}/T/Dirichlet',f'./dataset/para-{datasize}_new',level,0.999,0.999,datasize)
    get_pod_path.multilevel()

    prediction = get_new_pre(f'path/POD/{datasize}',f'dataset/{datasize}/POD',[10,50,150,10,30],level)
    ...
    # single_level = singlelevel(f'./dataset/{datasize}/T/Dirichlet/T1.mat',f'./dataset/para-{datasize}_new',0.999,datasize)
    # single_level.predict()
    # single_mean, single_base = single_level.get_meanbase()
    # result = single_level.get_newpre([10,50,150,10,30],single_mean, single_base)
    ...
    print(time)
    np.savetxt(f'result/time/TimeML{datasize}.txt',np.zeros([2,2])+time)
