...
tr_writer = SummaryWriter(log_dir="runs/mesh_data/train")
te_writer = SummaryWriter(log_dir="runs/mesh_data/test")
...
#         sumx = sumx + torch.sum(torch.abs(X))
#         sumy = sumy + torch.sum(torch.abs(Y))
...
def BRE(output,label):
    ...
    for i,item in enumerate(error):
        maxx = torch.max(label[i][0])
        maxy = torch.max(label[i][1])
        brex = brex + torch.mean(item[0])/maxx
        brey = brey + torch.mean(item[1])/maxy
    return brex, brey
...
class LayoutDataset(Dataset): 
    def __init__(self,train=True):
        data_input1 = sio.loadmat('./dataset/1000/bound/boundx2')['b']
        data_input2 = sio.loadmat('./dataset/1000/bound/boundy2')['b']
        data_output1 = sio.loadmat('./dataset/1000/mesh/meshx2')['b']
        data_output2 = sio.loadmat('./dataset/1000/mesh/meshy2')['b']
        ...
        if train:
            self.F = data_input[:800]
            self.u = data_output[:800]
        else:
            self.F = data_input[800:]
            self.u = data_output[800:]
    def __len__(self):
...
def train(model,train_loader,optimizer,epoch):
    ...
    for idx, (data, label) in enumerate(train_loader):
        ...
        Brex,Brey = BRE(output,label)
        loss = loss_function(output,label)
        loss1 = loss_function1(output,label)
        loss.backward()
        optimizer.step()
        brex += Brex.item()
        brey += Brey.item()
    ...
    print("Train Epoch: {}, train_loss: {}, mae: {}, learning_rate: {}, brex_mean: {}, brey_mean: {}".format(epoch,epoch_loss,total_loss1,optimizer.param_groups[0]['lr'],brex_mean,brey_mean))
...
def test(model,test_loader,epoch):
    ...
    print("Test Epoch: {}, test_loss: {}, mae_loss: {}, brex_mean: {}, brey_mean: {}".format(epoch,total_loss,total_loss1,brex_mean,brey_mean))
    return total_loss
...
for epoch in range(num_epochs):
    ...
    if loss < best_loss:
        best_loss = loss
        torch.save(net.state_dict(),"path/DeepLearning/best_mesh_data1000.pth")