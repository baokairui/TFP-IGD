...
def pde_loss(output,data,ref,h=1,n_iter=1):
    sumx = 0
    sumy = 0
    ...
    for i,item in enumerate(output):
        x = item[0] 
        y = item[1]
        # maxx = torch.max(ref[i][0])
        # maxy = torch.max(ref[i][1])
        maxx = 100
        maxy = 90
...
for epoch in range(num_epochs):
    ...
    if loss < best_loss:
        best_loss = loss
        torch.save(net.state_dict(),"path/Mesh/best_mesh_phyaccnorm2000.pth")